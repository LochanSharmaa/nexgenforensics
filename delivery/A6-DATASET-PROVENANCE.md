...
### CASIA — evaluation-side audit

For each evaluation image, similarity to the nearest training image.

<details><summary>Raw artefact — <code>train_eval_overlap.json</code></summary>
...
### CASIA — full-depth audit

Repeated at full identity coverage (105,631 images, all 10,572 identities). Five times the images found 2.7x the hits, confirming that sampling gives a floor rather than a complete answer.

<details><summary>Raw artefact — <code>overlap_casia_deep.json</code></summary>
...
### CASIA — exclusion list

The training-side inverse: for each training identity, similarity to the nearest evaluation image. **692 of 10,572 identities (6.5%) excluded** at threshold 0.40, chosen deliberately below the ~0.49 genuine-pair mean because excluding a clean identity is cheap and keeping a contaminated one makes every downstream number unfalsifiable.

<details><summary>Raw artefact — <code>exclusion_list.json</code></summary>
...
### QMUL-SurvFace — raw audit result

Reported 96.9% of identities above threshold. **This figure is an artefact and must not be quoted as contamination** — see the control below.

<details><summary>Raw artefact — <code>qmul_exclusion_list.json</code></summary>
...
### QMUL-SurvFace — the control that overturned it

Distinct QMUL directories are distinct people by construction, which makes a matched null directly measurable. The nearest DIFFERENT-PERSON QMUL image (0.600) scores HIGHER than the nearest TinyFace image (0.522), and a true same-person QMUL pair medians only 0.316. Degraded embeddings cluster by quality, not identity. **No identity overlap was established; nothing was excluded.**

<details><summary>Raw artefact — <code>qmul_overlap_control.json</code></summary>
...
### QMUL-SurvFace — capture characteristics

Confirms native low-resolution capture rather than downsampled clean imagery: median 27x22px, 84.1% below 32px high, smallest 7x5.

<details><summary>Raw artefact — <code>qmul_quality.json</code></summary>
